...
class AiService:
    ...
    def predict_best_team(self, my_party: List[str] = None, opponent_party: List[str] = None) -> Dict[str, Any]:
        """最適な選出と勝率を予測する"""
        my_party_names = my_party or []
        opponent_party_list = opponent_party or []

        if len(my_party_names) != 6 or len(opponent_party_list) != 6:
        ...
        result = predictor.predict_best_team(my_party_names, opponent_party_list)
