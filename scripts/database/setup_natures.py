...
from common import DB_PATH, NATURES_CSV_PATH, SCHEMA_DIR

def setup_natures():
    """
    naturesテーブルを再作成し、データを投入する (DROP, CREATE, INSERT)
    """
    table_name = "natures"
    ...
    # --- ファイル存在チェック ---
    for path in [NATURES_CSV_PATH, DB_PATH, schema_path]:
    ...
    try:
        ...
        natures_df = pd.read_csv(NATURES_CSV_PATH)
        
        db_columns = ["id", "name", "name_ja", "increased_stat", "decreased_stat"]
        data_to_insert = [tuple(row) for row in natures_df[db_columns].itertuples(index=False)]
