...
from common import DB_PATH, MOVES_CSV_PATH, SCHEMA_DIR
...
def setup_moves():
    ...
    # --- ファイル存在チェック ---
    for path in [MOVES_CSV_PATH, DB_PATH, schema_path]:
    ...
    try:
        ...
        data_to_insert = [tuple(row) for row in moves_df[db_columns].itertuples(index=False)]
