...
from common import MASTER_DATA_DIR, NATURES_CSV_PATH

def generate_natures_csv():
    """PokeAPIから性格データを取得し、CSVファイルとして保存する"""
    print("--- 性格CSVファイルの生成開始 ---")
    os.makedirs(MASTER_DATA_DIR, exist_ok=True)
    endpoint = "nature"
    path = NATURES_CSV_PATH
    header = ["id", "name", "name_ja", "increased_stat", "decreased_stat"]
    ...
    try:
        with open(path, "w", newline='', encoding="utf-8") as f:
            ...
            for i, res in enumerate(resource_list):
                ...
                writer.writerow([data['id'], item_name, get_japanese_name(data), inc_stat, dec_stat])
        print(f"\n'{path}' を作成しました。")
    except Exception as e:
        print(f"\n性格データの生成中にエラーが発生しました: {e}")
    finally:
        print("--- 性格CSVファイルの生成完了 ---")

if __name__ == '__main__':
    generate_natures_csv()
